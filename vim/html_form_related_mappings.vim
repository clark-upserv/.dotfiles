" Html Show edit swap
  ...
  nnoremap <silent> ,hsbs a<div class="d-flex"><return><div id="ChangeSesId-ses-show" class="ChangeSesId-ses-edit-button ses-single-show"><return><span><%= ChangeDisplay %></span><return></div><return><return><div id="ChangeSesId-ses-edit"><return><%= render('ChangePath/ChangeSubController/edit', ChangeObject: ChangeObject) %><return></div><return></div><esc>/ChangeSesId\\|ChangeDisplay\\|ChangePath\\|ChangeSubController\\|ChangeObject<return>
...
" Html Form
  ...
  nnoremap <silent> ,hfsi a<%= form_with(scope: 'ChangeScope', url: ChangePath_path(ChangeObject), method: :ChangeMethod) do \|ChangeForm_form\| %><return><div class="d-flex align-items-center"><return><%# DeleteThis - use text feild or change input type %><return><%= ChangeForm_form.text_field(<return>  :ChangeAttribute,<return># DeleteThis - typically looks better to manage input witdh by setting html size attribute; otherwise input width will default to 100% and doesn't look very good usually<return><backspace><backspace>size: ChangeObject.ChangeAttribute.length,<return># DeleteThis - include class "page-head" if for page header<return>DeleteThis - include class "card-head" if for card header<return><backspace><backspace>class: 'form-control ses-single-input',<return># DeleteThis - goal is for edit form to look like show but with border and buttons. If above classes do not work, try adding custom styles:<return><backspace><backspace># DeleteOrUseThis - style: 'height: XXpx !important; font-size: XXpx; font-weight: XXX; Etc...',<return><backspace><backspace><esc>:call HtmlInputMainAttributes()<return>o) %><return>  <a class="pl-3 pr-2 fs-4 c-pointer text-secondary mdi mdi-close-circle-o ChangeSesId-ses-cancel-button"></a><return><button class="pl-2 pr-3 fs-4 btn-no-background text-primary mdi mdi-check-circle do-not-disable"></button><return></div><return><%= render('shared/inline_errors', errors: ChangeObject.errors.messages[:ChangeAttribute]) %><return><% end %><esc>:call HtmlInputSearch()<return>
